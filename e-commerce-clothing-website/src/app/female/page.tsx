...
export default async function Female() {
  const femaleProduct = await getProducts();
  const femaleFilterProduct = femaleProduct.filter(
    (products: any) => products.productCategory === "Female"
  );

  return (
    <>
      <div className="mx-0 md:mx-5 min-h-screen">
        <div className="max-w-[1850px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-5 gap-x-3 min-h-screen">
          {femaleFilterProduct.map((product: any) => (
            <>
